  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => AppFieldComponent),
      multi: true
    }
  ]
})
export class AppFieldComponent implements ControlValueAccessor {
  control!: FormControl;
  ...
  @Input() messages: { [key: string]: string; } | undefined;
  ...
  mapErrors() {
    if (this.control.errors) {
      Object.keys(this.control.errors).forEach((key: any) => {
        if (this.messages) {
          let errorMessage = this.messages[key] ? this.messages[key] : this.control.errors?.[key];
